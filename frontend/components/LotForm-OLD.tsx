...
interface LotFormProps {
  ...
  showHeader?: boolean; // New prop to toggle header visibility
}
...
const LotForm: React.FC<LotFormProps> = ({
  ...
  const [isRegisteredForThisProduct, setIsRegisteredForThisProduct] =
    useState(false);
  const [lotteryUserDocumentId, setLotteryUserDocumentId] = useState<
    string | null
  >(null);
    ...
    try {
      ...
      if (response?.data) {
        ...
        const registeredEntry = response.data.find((entry: any) =>
          entry.products.some(
            (product: any) => product.documentId === productId
          )
        );
  ...
  const handleUnregister = async () => {
    ...
    try {
      const response = await fetchAPI(
        `/lottery-users/${lotteryUserDocumentId}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (response.ok) {
        ...
        throw new Error(
          response?.error?.message || "Failed to unregister from lottery."
        );
      }
    } catch (error) {
